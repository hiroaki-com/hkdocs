...
でも、いざ動画を変換しようとすると「SaaSはサブスクが必要」「ローカルツールは重い」「ffmpegコマンドを毎回調べるのがめんどくさい」という壁にぶつかります。
...
-   **SaaSツール** : ブラウザで完結して便利だけど、品質設定を細かく触ろうとすると有料プランが必要だったりする
-   **ローカルのGUIツール** : ダウンロードして使えるものもあるけど、OSのアップデートのたびに動かなくなったりして管理が煩わしい
-   **ffmpegを直接叩く** : 一番自由度が高いけど、オプションを毎回調べるのが手間。`palettegen`と`paletteuse`の書き方、いつも忘れる

エンジニアとしては、動画変換ツールの使い方をキャッチアップすることは学習の本質ではないし、SaaSにお金を払い続けるのも気が引けます。どうせなら自分で作って、好きなように使い回せるものにしようと思いました。

Google Colabなら無料でffmpegが動かせるし、UIをノートブック上に作れば毎回コマンドを書く必要もない。そういう発想でこのツールを作りました。

### 使い方

Pythonのコードを書く必要はありません。セルを上から順に実行して、UIを操作するだけです。

#### ① セットアップ

最初のセルを実行するだけ。ffmpegのインストールが自動で完了します。

#### ② 動画をアップロード

ファイル選択ダイアログが開くので、変換したい動画を選択します。複数ファイルを選択すれば、まとめて結合することもできます。

対応入力形式：`.mov` `.mp4` `.avi` `.mkv` `.webm`

#### ③ 結合順を指定（複数ファイルの場合）

複数の動画をアップロードした場合、ドロップダウンで結合する順番を指定します。1本だけの場合はこのステップはスキップされます。
...
| プリセット | 横幅 | FPS | 色数 | 用途 |
|:---|:---:|:---:|:---:|:---|
| GitHub README用 | 960px | 15fps | 256色 | README埋め込み（推奨） |
...
再生速度も変更できます。1.5倍速や2.0倍速にすると、長い操作デモを短くまとめるのに便利です。

#### ⑤ 変換実行

再生ボタン（▶）を押すと変換が始まります。設定を変えてやり直したいときは、④の設定を変更してからこのセルだけ再実行すればOKです。

#### ⑥ プレビュー確認

変換完了後、ノートブック上でそのままプレビューを確認できます。GIFはインラインで表示、MP4はプレイヤーが埋め込まれます。

#### ⑦ 保存

保存先をラジオボタンで選択して実行します。
...
-   設定変更と再実行の分離
    ...
    ④の設定セルと⑤の変換実行セルを分けているので、品質設定を変えて再変換したいときは⑤だけを再実行すれば済みます。毎回①から実行し直す必要がありません。
...
「ffmpegのコマンドを毎回調べるのが嫌だ」「変換ツールのサブスクを払いたくない」「ローカルに余計なソフトを入れたくない」、その三拍子が揃ったので自作しました。

Google Colabのノートブックという形にしたことで、ブラウザさえあればどこでも使えて、設定もGUIで完結します。個人的には、これ以降READMEへのGIF添付が格段に楽になりました。

同じように「デモ動画の変換、毎回面倒だな」と感じている方の参考になれば嬉しいです。

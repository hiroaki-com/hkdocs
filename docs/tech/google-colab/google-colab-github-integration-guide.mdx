...
本記事は継続的にメンテナンスしている手順書です。執筆当時の背景や試行錯誤の記録は [ブログ版の記事](/blog/2025/08/08/google-colab-github-integration-guide/) を参照してください。
...
![GitHubリポジトリの作成](/img/colab_github_01_create_new_repo.webp)
...
![メニューからGitHubにコピーを保存を選択](/img/colab_github_02_menu_save_copy_to_github.webp)
...
![GitHub認証プロセス](/img/colab_github_03_auth_process.webp)
...
![保存先設定ダイアログ](/img/colab_github_04_save_settings_dialog.webp)
...
![GitHub Codeボタン](/img/colab_github_05_clone_code_button.webp)
...
![VSCodeテキストエディター選択](/img/colab_github_07_vscode_select_text_editor.webp)

![VSCode右クリックメニュー](/img/colab_github_06_vscode_right_click_menu.webp)
...
![Colab ノートブックを開く GitHubタブ](/img/colab_github_08_open_notebook_github_tab.webp)

    -   対象のリポジトリを選択（またはURLで検索）し、最新版に更新されたノートブックを開いて実行します。

![Colab リポジトリ検索](/img/colab_github_09_search_repo_url.webp)
...
- `ファイル` > `GitHub にコピーを保存`（推奨）
- 継続作業が必要な場合は `ファイル` > `ドライブにコピーを保存` でも可
- 保存後、ローカル側で `git pull origin main` を実行して同期する
- Drive 保存版から直接作業を開始しないこと
...
1. `git pull` 実行時にコンフリクトを検知
2. VSCode / Cursor 等の差分比較ツールで修正
3. 修正後 `commit` → `push`

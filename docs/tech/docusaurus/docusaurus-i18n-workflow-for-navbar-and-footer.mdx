...
last_update:
  date: 2025-06-16
tags: [docusaurus, i18n, react, typescript, ci/cd, チュートリアル]
...
本記事は継続的にメンテナンスしている手順書です。執筆当時の背景や試行錯誤の記録は [ブログ版の記事](/blog/2025/06/21/docusaurus-i18n-workflow-for-navbar-and-footer/) を参照してください。
...
1.  CLIコマンドの実行。ターミナルで以下のコマンドを実行し、追加したテキストを翻訳対象として抽出。
    ...
    :::info 翻訳ファイルの自動更新
    このコマンドは、`docusaurus.config.ts`の変更内容をスキャンし、`i18n/en/docusaurus-theme-classic/`ディレクトリ配下のJSONファイルに、翻訳が必要な項目を自動で追加・更新します。
    :::

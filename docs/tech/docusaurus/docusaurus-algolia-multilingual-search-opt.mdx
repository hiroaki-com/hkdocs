...
last_update:
  date: 2026-07-06
tags: [docusaurus, search, i18n, チュートリアル]
description: "Docusaurus 多言語サイトで Algolia 検索の言語混在問題を解決する方法。サイトの表示言語と連動した最適な検索体験を実現するための具体的な設定手順を解説します。"
---
...
本記事は継続的にメンテナンスしている手順書です。執筆当時の背景や試行錯誤の記録は [ブログ版の記事](/blog/2025/06/24/docusaurus-algolia-multilingual-search-opt/) を参照してください。

本記事では、この問題を解決し、**サイトの表示言語と連動した最適な検索体験**を実現するための具体的な設定手順を備忘録として整理します。

:::warning 2026-07 追記：本手法の重要な訂正
本記事の核である「`contextualSearch` がロケールに応じてインデックス名（`yoursite_ja` / `yoursite_en`）を組み立てて検索する」という説明は、Docusaurus の実装を確認した結果、**誤り**でした。実際の `contextualSearch` は **`indexName` を変更せず**、単一のインデックスに対しクエリ時に `language:<locale>` などの facetFilters を付与して絞り込みます。

そのため本記事の言語別インデックスはフロントエンドから一切参照されず、クローラーだけが更新し続ける状態になっていました。当サイトはこの検証を踏まえ、**単一インデックス構成に統合済み**です。言語混在の防止は `contextualSearch: true` のみで実現でき、言語別インデックスの分離は不要です。単一インデックスでも `ignorePlurals: ["en"]` のように言語配列指定で言語別チューニングが可能です。訂正後の正しい手順は [DocusaurusのAlgolia検索を単一インデックス構成で精度改善する](/docs/tech/docusaurus/docusaurus-algolia-single-index-search-precision/) を参照してください。
:::

...
そのため本記事の言語別インデックスはフロントエンドから一切参照されず、クローラーだけが更新し続ける状態になっていました。当サイトはこの検証を踏まえ、**単一インデックス構成に統合済み**です。言語混在の防止は `contextualSearch: true` のみで実現でき、言語別インデックスの分離は不要です。単一インデックスでも `ignorePlurals: ["en"]` のように言語配列指定で言語別チューニングが可能です。
:::

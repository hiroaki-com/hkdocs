...
本記事は継続的にメンテナンスしている手順書です。執筆当時の背景や試行錯誤の記録は [ブログ版の記事](/blog/2025/06/16/implementing-i18n-in-docusaurus/) を参照してください。
...
2.  **コンテンツのコピーと翻訳**
    <Admonition type="tip" title="アドバイス">
      最初は**最低1ページ**だけ翻訳し、i18n機能が正しく動作するかを早期に確認することを強く推奨します。
    </Admonition>
...
<Admonition type="note" title="注記">
  開発サーバー (`start` コマンド) は、一度に一つの言語しか起動できません。また、Docker環境で外部からアクセスするには `--host 0.0.0.0` オプションが必須です。
</Admonition>

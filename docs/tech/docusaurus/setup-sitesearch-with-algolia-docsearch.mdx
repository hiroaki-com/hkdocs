...
本記事は継続的にメンテナンスしている手順書です。執筆当時の背景や試行錯誤の記録は [ブログ版の記事](/blog/2025/06/10/setup-sitesearch-with-algolia-docsearch/) を参照してください。
...
3.  **コンテンツの自動更新**
    ...
    <Admonition type="warning" title="注意">
      クロールが完了し、インデックスにデータが登録されるまで検索機能は動作しない。
    </Admonition>
...
2.  設定項目`renderJavaScript: false`を`true`に変更。
    ...
    ```javascript title="Algoliaクローラー設定ファイル"
    // Algoliaクローラー設定ファイル
    new Crawler({
      // ...

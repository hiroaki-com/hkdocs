...
last_update:
  date: 2025-06-18
tags: [Google Docs, Apps Script, GAS, チュートリアル]
description: "Google ドキュメントで特定の文字列を検知して自動的に改ページを挿入する Apps Script の解説。長文ドキュメント編集の繰り返し作業を自動化する効率化スクリプトです。"
---
...
### 使い方
1.  Googleドキュメントを開き、「拡張機能」>「Apps Script」からスクリプトエディタを開きます。
2.  以下のスクリプトをエディタに貼り付けます。
3.  スクリプト内の設定箇所を編集し、保存・実行します。

<Admonition type="info" title="スクリプトの設定方法">
<p>
スクリプトを貼り付けた後、コードの先頭にある <code>PAGEBREAK_PREFIXES</code> の値を、改ページの目印としたい文字列の配列に変更してください。
</p>
<ul>
    <li>この設定は<strong>前方一致</strong>で動作します。</li>
    <li>配列には、目印となる文字列を1つ以上設定してください。（例: <code>["Chapter ", "図-"]</code>）</li>
    <li>大文字・小文字は区別されます。</li>
    <li>コード内のコメントに、より詳しい設定方法が記載されています。</li>
</ul>
<p>例：<code>const PAGEBREAK_PREFIXES = ["問題文 (日本", "章"];</code></p>
</Admonition>

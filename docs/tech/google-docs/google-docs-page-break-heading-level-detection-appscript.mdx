...
description: "Google ドキュメントで見出しレベルを検知して自動的に改ページを挿入する Apps Script の解説。長文ドキュメント編集の繰り返し作業を自動化する効率化スクリプトです。"
---
...
<p>
スクリプトを貼り付けた後、コードの先頭にある <code>TARGET_HEADING_LEVEL</code> の値を、改ページを挿入したい見出しレベル（1〜6の整数）に変更してください。<br/>
コード内のコメントに詳しい設定方法が記載されています。
</p>
<p>例：<code>const TARGET_HEADING_LEVEL = 3;</code> // 「見出し 3」の前に改ページを挿入する場合</p>
</Admonition>

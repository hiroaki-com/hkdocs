...
本記事の「`contextualSearch` がロケールに応じてインデックス名（`yoursite_ja` / `yoursite_en`）を組み立てて検索する」という説明は**誤り**でした。実際の `contextualSearch` は `indexName` を変更せず、単一インデックスに対しクエリ時に `language:<locale>` などの facetFilters で絞り込みます。当サイトは単一インデックス構成に統合済みです。詳細は[技術ドキュメント版](/docs/tech/docusaurus/docusaurus-algolia-multilingual-search-opt/)の追記を参照してください。
:::

...
description: "Docusaurus 多言語サイトで Algolia 検索の言語混在問題を解決する方法。サイトの表示言語と連動した最適な検索体験を実現するための設定手順を解説します。"
---
...
本記事は執筆当時の記録です。継続的にメンテナンスしている手順は [技術ドキュメント版](/docs/tech/docusaurus/docusaurus-algolia-multilingual-search-opt/) を参照してください。

本記事では、この問題を解決し、**サイトの表示言語と連動した最適な検索体験**を実現するための具体的な設定手順を備忘録として整理します。

:::warning 2026-07 追記：本手法の重要な訂正
本記事の「`contextualSearch` がロケールに応じてインデックス名（`yoursite_ja` / `yoursite_en`）を組み立てて検索する」という説明は**誤り**でした。実際の `contextualSearch` は `indexName` を変更せず、単一インデックスに対しクエリ時に `language:<locale>` などの facetFilters で絞り込みます。当サイトは単一インデックス構成に統合済みです。訂正後の正しい手順は [DocusaurusのAlgolia検索を単一インデックス構成で精度改善する](/docs/tech/docusaurus/docusaurus-algolia-single-index-search-precision/)、その経緯は [ブログ版の記録](/blog/2026/07/06/algolia-search-precision-single-index/) を参照してください。
:::

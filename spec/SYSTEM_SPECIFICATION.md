...
III. システムアーキテクチャ
-------------------------
  本システムは、開発者によるGitHubへのコードプッシュを起点とする。
  GitHub Actionsがこれを検知し、プッシュされたブランチに応じてステージング環境
  または本番環境へ自動的にデプロイを実行する。このプロセスには、Dockerイメージの
  ビルド、Artifact Registryへのプッシュ、Cloud Runへのデプロイが含まれる。
  エンドユーザーはCloud Run上で稼働するサイトにアクセスし、Algoliaによる
  高速な全文検索機能を利用できる。
...
----------------------------------------
  ...
  * ブランチ戦略:
    - `main`: 本番環境にデプロイされる安定版ブランチ。直接のコミットは禁止し、

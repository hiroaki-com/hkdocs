### HkDocsの仕様書 (2025/07/03 版)

I. 基本情報

1.  プロジェクト名: hkdocs
2.  ドメイン: `hkdocs.com`
3.  プロジェクト目的: 技術ブログ、学習記録(Docs)、日記(Diary)、ニュースリンク集などを集約した個人用ナレッジベースの構築と公開。
...
*   コミットメッセージ: 変更内容の可読性を高め、履歴の追跡を容易にするため、コミットメッセージは[Conventional Commits](https://www.conventionalcommits.org/ja/v1.0.0/)の規約を参考とする。
*   Pull Request (PR) 運用:
...
*   **Cloud Run**:
    ...
    *   設定:
        *   サービス名: `hkdocs-service` (本番用)ブランチのデプロイ先として設定
...
*   カスタムページ:
    *   ニュース (`src/pages/news.tsx`): 日々の情報収集に役立つ、国内外の主要ニュースサイトへのリンクをカテゴリ別にまとめたページ。

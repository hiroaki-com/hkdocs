...
description: "A Google Colab tool using Cloudflare Tunnel to create a private LLM chat environment that keeps conversation logs off external servers, running the latest Ollama models."
---
...
The release announcement for this tool is available as a [blog post](/en/blog/2026/03/27/release-colab-ollama-private-chat/).
...
This tool builds on a free Ollama server running on Colab. If you haven't set one up yet, start with [running Ollama on Google Colab's free GPU](/en/docs/tech/google-colab/google-colab-ollama-free-llm-server/). To compare models before committing to one, see the [multi-model benchmarking tool](/en/docs/tech/google-colab/google-colab-ollama-multi-model-benchmarker/).

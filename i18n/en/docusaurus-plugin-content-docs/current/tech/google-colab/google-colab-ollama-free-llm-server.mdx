---
title: "Run Ollama on Google Colab's Free GPU: Free LLM Server via ngrok"
authors: [hk]
tags: [llm, google-colab, python, claude-code, automation]
description: "Run Ollama on Google Colab's free GPU and expose it via ngrok to create a completely free LLM server usable from Claude Code, Continue, and other assistants."
---
...
The release announcement for this tool is available as a [blog post](/en/blog/2026/03/01/release-colab-ollama-server/).
...
If you'd like to compare models before picking one, check out the [multi-model benchmarking tool](/en/docs/tech/google-colab/google-colab-ollama-multi-model-benchmarker/) as well. And once the server is running, you can layer a private web chat UI on top of it — see [building a private LLM chat environment on Colab with Cloudflare Tunnel](/en/docs/tech/google-colab/google-colab-ollama-private-chat-for-local-llm/).
